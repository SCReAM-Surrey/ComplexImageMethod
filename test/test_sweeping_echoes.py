...
# helper function
# =============================================================================
def plot_spectrogram(rir, fs, fft_size, fft_hop, fft_zp, analysis_window, method, saveFlag=False, absorp=0):
    ...
    if saveFlag:
        plt.savefig('../figures/spec_'+method+'_absorp=' + str(absorp) + '.png', dpi = 1000)
...
fig,ax = plt.subplots()
fig.set_size_inches(3.3,2.5)
ax1, = ax.semilogx(wave_nums*(c/(2*np.pi)), 20*np.log10(np.abs(pressure_at_receiver)))
...
#  plot spectrograms
plot_spectrogram(rir_cism, fs, fft_size, fft_hop, fft_zp, analysis_window, 'CISM', True, np.round(absorp,3))
